...
def fetch_survey_data():
    # Simulated survey feedback data
    data = [
...
def dynamic_join(usage_df, survey_df, join_key="user", how="inner"):
    # Perform dynamic join on specified key and type
    return pd.merge(usage_df, survey_df, on=join_key, how=how)

def analyze_usage(df):
    # Basic analysis: usage count per tool and average satisfaction
...
def generate_usage_chart(df, chart_type="bar"):
    # Generate a Plotly chart and return as a base64-encoded PNG image string
    ...
    if chart_type == "bar":
        ...
        # Example: usage over time (requires timestamp parsing)
    ...
    # Convert figure to PNG image in base64 to embed in JSON response
